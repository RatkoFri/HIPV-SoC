module obi_uart #(
    ...
    // HIPV FIX (also apply in the obi_uart repo): tx_start delayed by one
    // cycle. The tx buffer register (uart_tx_buffer) and the transmitter
    // FSM's data latch both fire on the write edge, so with
    // tx_start = wr_en[2] the FSM captures the buffer's PRE-write value
    // and every byte is transmitted one write late (first frame = 0x00).
    // Verified in tb/unit/uart: frames=['0x0','0xa3'] before the fix,
    // ['0xa3','0x5c'] after. Delaying tx_start lets the FSM sample the
    // buffer after it has updated.
    logic tx_start_q;
    always_ff @(posedge clk_i) begin
        if (!rstn_i) tx_start_q <= 1'b0;
        else tx_start_q <= wr_en[2];
    end
    ...
    uart_system uart_inst (
        ...
        .tx_start(tx_start_q), // HIPV FIX: was wr_en[2] (see comment above)
